...
## READI Agent Architecture

The READI Agent is built as a multi-agent orchestration pipeline designed to solve complex compliance auditing tasks. It combines structured database data, unstructured procedural knowledge, and external web research into a cohesive "Auditor" persona.
...
2. **Intent Classification**: 
   ...
   - **Categorization**: Classifies query into `DATABASE` (Facts), `PROCEDURE` (RAG), and/or `WEB_SEARCH` (External).
   - **Auditor Rule**: If auditing terminology is detected (audit, violation, compliance), it mandates a multi-intent lookup across both database and procedures.
3. **Data Retrieval (Parallel Processing)**:
   - **Database Query Planner**: 
     - Selects relevant tables (e.g., `pilot_mission`, `alert_log`).
     - Generates a JSON query plan matching the user's role-based permissions.
     - Executes filtered Postgres queries via Supabase with strict tenant isolation (`ownerID`).
   - **Procedure Retrieval (RAG)**:
     - Generates an embedding for the question.
     - Performs semantic search using `pgvector` against ingested mission manuals and FRD documents in Supabase.
   - **Web Researcher**: 
     - Searches SerpAPI for the latest drone regulations or industry standards if internal data is insufficient.
4. **Audit Synthesis**:
   - The "Compliance Auditor" agent receives raw data from all sources + a persistent set of "Audit Rules" (e.g., 120m altitude cap, no-fly during high wind alerts).
   - It performs cross-table correlation (e.g., matching a flight's `actual_start` with an alert's timestamp).
5. **Direct Response**:
   - Outputs a concise audit report, flagging safety breaches with a clear `VIOLATION:` prefix.

### Security & Compliance
- **Role-Based Access Control (RBAC)**: Enforced at the API level, restricting visible tables and columns based on the user's role (e.g., OPM vs. PIC).
- **Data Isolation**: All database queries are automatically scoped to the user's `ownerID` (Organization ID).
- **Deterministic Evidence**: While the response is generated by an LLM, the logic is strictly anchored to raw "Evidence" data fetched from source tables.